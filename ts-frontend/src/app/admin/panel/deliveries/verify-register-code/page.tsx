...
export default function VerifyAdminDeliveryRegisterCode() {
  ...
  const handleVerifyAdminDeliveryRegisterCode = async () => {
    setLoading(true);
    try {
      // Yup validation
      resetErrors();
      ...
      axiosAdmin
        .post(`auth/delivery/verify-register-code`, {
          phone: values.phone,
          code: values.code,
        })
        .then((res) => {
          router.push("/admin/panel/deliveries/register");
  ...
  return (
    <div className="flex flex-col self-start items-center p-[10px] rounded-xl shadow border border-gray-200 gap-y-5 relative w-full max-w-lg mx-auto bg-white">
      ...
      <Button
        onClick={() => {
          handleVerifyAdminDeliveryRegisterCode();
        }}
        disabled={loading}
